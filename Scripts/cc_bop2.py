...
# 添加木桌+蓝色箱子模型对象，命名LBox_obj_model
...
for d in bpy.context.preferences.addons["cycles"].preferences.devices:
    # only use GPU.
    if d["name"] == 'Genuine Intel CPU 0000 @ 2.10GHz':
        d["use"] = 0
    else:
        d["use"] = 1
    print(d["name"], d["use"])
...
#     # 下面这句话相当于lefuqiu = bpy.data.objects['lefuqiu']
...
# bpy.data.objects['Camera'].select_set(1)  # 设置视野
...
def sample_pose(obj: bproc.types.MeshObject):
    ...
    obj.set_rotation_euler(bproc.sampler.uniformSO3())  # 这可能只产生z轴旋转
...
bproc.object.sample_poses(
    # objects_to_sample=target_bop_objs + sampled_distractor_bop_objs,  # 把物体列表放进去
    objects_to_sample=target_bop_objs,  # 把物体列表放进去
    sample_pose_func=sample_pose_sky,  # 调用空中姿态采样
...
# 设置整体乐芙球对象的刚体主动，指定所有物理属性（如质量、摩擦力等）
for obj in target_bop_objs:
...
while cam_poses < 5:
    ...
    # 检查障碍物是否距离相机至少0.3米，并确保视野足够有趣
    if bproc.camera.perform_obstacle_in_view_check(cam2world_matrix, {"min": 0.3}, bop_bvh_tree):
        # 坚持相机姿势
        bproc.camera.add_camera_pose(cam2world_matrix, frame=cam_poses)
        cam_poses += 1

# #### 要生成bop数据集 必须给场景中的每个物体赋值属性category_id #############################################
# id = 100
# for obj in bpy.data.objects:
...
# 渲染以 COCO 格式包含实例分割,polygon解决mask形变问题
